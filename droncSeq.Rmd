...
tissue <- CreateSeuratObject(raw.data = tissue,  project = "hip_pcf")
levels(tissue@ident)
```
```{r}
dim(tissue@raw.data)
...
# AddMeta: total reads 
tissue <- AddMetaData(object = tissue,metadata = colSums(tissue@raw.data),col.name = "total.reads")
VlnPlot(object = tissue, features.plot = c("nGene", "nUMI", "percent.mito","total.reads"), nCol = 4)
...
# We filter out cells that have unique gene counts less than 200 and total reads less than 10,000
...
tissue <- FilterCells(object = tissue, subset.names = c("nGene", "total.reads"), low.thresholds = c(200, 10000), high.thresholds = c(Inf, Inf))
```
***
...
tissue <- FindVariableGenes(object = tissue, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.005,  y.cutoff = 0.2)
```
...
tissue <- ScaleData(object = tissue, vars.to.regress = c("nUMI", "nGene"))
```
***
