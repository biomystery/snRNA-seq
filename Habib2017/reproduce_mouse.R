...
dat <- dat %>% separate(ClusterID,into=c("ClusterID","Cluster"),sep="_")
dat.sum <- dat%>% group_by(Cluster.ID) %>% summarise(num=n())
dat <- dat%>% unite(col = "Clusters",c("ClusterID","Cluster"),remove = F)
...
saveRDS(data.all.long,file = "mouse_all.Rds")
